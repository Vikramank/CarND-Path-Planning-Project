    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double min_safe_distance = 50.0; // minimum bumber to bumper distance
                                           // in order to avoid any possible
                                           // collision
          double min_safe_distance_lane =
              60.0; // minimum absolute distance between our car and cars in
                    // adjacent lanes to perform a lane shift
          double very_close_distance =
              10.0; // activate emergency breaking to avoid collision
          bool try_lane_change = false; // a variable to perform lane change
          bool left_lane_clear =
              false; // to ensure lane change left is possible
          bool right_lane_clear =
              false; // to ensure lane change right is possible
          ...
          for (int i = 0; i < sensor_fusion.size(); i++) {
            ...
            if (d < (2 + 4 * lane + 2) && d > (2 + 4 * lane - 2)) {
              check_car_s += (double)prev_size * 0.02 *
                             check_speed; // position of car in s coordinate
                                          // after a time interval
                                          // check for cars in our lane
              if ((check_car_s > car_s) &&
                  (abs(check_car_s - car_s) < min_safe_distance)) {
                too_close = true;

                std::cout << "Car infront me with speed !!!" << check_speed
                          << '\n';
                try_lane_change = true;

                if (abs(check_car_s - car_s) < very_close_distance) {
                  std::cout << "Activating emergency breaking" << '\n';
                  ref_vel = 10.0;
                }
              }
            }
          ...
          // Lane change
          if (try_lane_change) { // preference to lane change left to stay near
                                 // the yellow line
            if ((too_close_left == false) and (lane != 0)) {
              lane = lane - 1;
              cout << "Changing lane to left lane" << endl;
            }
            // if left lane change is not possible, try lane change right
            else if ((too_close_right == false) and (lane != 2)) {
          ...
          // 4 points spaced between 30m points to make a smooth transition.
          ...
          vector<double> next_wp2 =
              getXY(car_s + 90, (2 + 4 * lane), map_waypoints_s,
                    map_waypoints_x, map_waypoints_y);
          // vector<double> next_wp3 = getXY(car_s + 120, (2 + 4 * lane),
          ...
          double N =
              target_dist /
              (0.02 * ref_vel /
               2.24); // 2.24 is the conversion factor to convert mph to m/s
